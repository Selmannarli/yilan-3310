...
function MiniGamePanel({ game, playerId, players, send }: { game: MiniGameState; playerId: string; players: Player[]; send: (message: object) => void }) {
  ...
  if (game.phase === "selecting") return <section className="mini-stage"><div className="mini-title"><span>1</span><div><small>RAKİP SEÇ</small><h2>Düelloya kimi çağırıyorsun?</h2></div></div>{playerId===game.challengerId?<div className="opponent-grid">{players.filter(p=>p.id!==playerId&&p.connected).map((p,i)=><button key={p.id} onClick={()=>send({type:"selectMiniOpponent",opponentId:p.id})}><span className={`avatar ${colors[(i+1)%4]}`}>{initials(p.nickname)}</span><b>{p.nickname}</b><i>→</i></button>)}</div>:<div className="mini-wait"><span className="dots"><i/><i/><i/></span><b>{name(game.challengerId)}</b> rakibini seçiyor</div>}</section>;
  if (game.phase === "ready") return <section className="mini-stage ready-stage"><div className="versus"><div><span className="avatar purple">{initials(name(game.challengerId))}</span><b>{name(game.challengerId)}</b></div><i>VS</i><div><span className="avatar orange">{initials(name(game.opponentId!))}</span><b>{name(game.opponentId!)}</b></div></div>{participant?<button className="mini-ready" disabled={game.readyIds.includes(playerId)} onClick={()=>send({type:"miniReady"})}>{game.readyIds.includes(playerId)?"HAZIRSIN ✓":"HAZIRIM"}</button>:<p>Oyuncular hazırlanıyor...</p>}<small>{game.readyIds.length}/2 HAZIR</small></section>;
      ...
      {!room ? (
        ...
        <>
          ...
          <section className="game-area">
            ...
            {isDigital&&room.miniGame ? <MiniGamePanel game={room.miniGame} playerId={playerId} players={orderedPlayers} send={send}/> : isVote ? (
